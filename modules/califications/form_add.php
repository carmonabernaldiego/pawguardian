...
<div class="form-data">
    ...
    <div class="body">
        <form name="form-update-groups" action="insert.php" method="POST" autocomplete="off" autocapitalize="on">
            <div class="wrap">
                <div class="first">
                ...
                <div class="content-full">
                    <table class="student-grades-table">
                        ...
                        <tbody>
                        <?php
                        $associatedStudentsSql = "SELECT student FROM groupschool_students WHERE id_group = '" . $_SESSION['group_id'] . "' AND school_period = '" . $_SESSION['group_school_period'] . "'";

                        if ($associatedStudentsResult = $conexion->query($associatedStudentsSql)) {
                            while ($associatedStudentRow = mysqli_fetch_array($associatedStudentsResult)) {
                                $studentId = $associatedStudentRow['student'];

                                $studentInfoSql = "SELECT user, name, surnames FROM students WHERE user = '$studentId'";

                                if ($studentInfoResult = $conexion->query($studentInfoSql)) {
                                    if ($studentInfoRow = mysqli_fetch_array($studentInfoResult)) {
                                        ...
                                        // Si se encontraron calificaciones, mostrarlas en los campos
                                        if ($calificationsRow) {
                                            ...
                                            // Si no se encontraron calificaciones, mostrar campos vacíos
            ...
            <button id="btnSave" class="btn icon" name="btn" value="form_add_group" type="submit">save
            </button>
        </form>
    </div>
</div>
